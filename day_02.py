...
if you were discovered - and use your fancy wrist device to quickly scan every box and produce a list
...
def get_checksum(box_ids: list) -> int:
    """
    Counts number of duplicate values of which hold 2 or 3 duplicate letters

    Arguments:
        box_ids (list): List of box IDs captured
    """
...
def get_matching_boxes(box_ids: list) -> str:
    """
    Iterates through each box IDs permutation and locates the IDs which differ by one character at the same index
    and returns the common characters between both IDs

    Arguments:
        box_ids (list): List of box IDs captured
    """

    for perm in itertools.permutations(box_ids, 2):
        c = 0
        matches = list()

        # Iterate through each permutation to find if item has one matching character set at the same index
        for x, y in zip(*perm):
            if x != y:
                c += 1
                if c > 1:
...
if __name__ == "__main__":
    ...
    inputs = open("puzzle_inputs/input_day_2.txt", "r").readlines()
    print(get_checksum(inputs), get_matching_boxes(inputs))
