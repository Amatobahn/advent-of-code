...
def calculate_overlap(claim_items: list, fabric: np.ndarray) -> int:
    """
    Get the overlapping count of indices that are greater than one.

    Arguments:
        claim_items (list): List of elf claims
        fabric (np.ndarray): Fabric shape
    """

    for claim in claim_items:
        index, x, y, w, h = map(int, re.findall(r'\d+', claim))
        claim_container = fabric[x: x+w, y: y+h]
        claim_container[:] = claim_container + 1

    return int(np.sum(np.where(fabric > 1, 1, 0)))


def get_non_overlap_id(claim_items: list, fabric: np.ndarray) -> int:
    """
    Find's the sole elf claim ID that is not overlapping with any other claim IDs
    ...
    for claim in claim_items:
        claims.append([int(data) for data in map(int, re.findall(r'\d+', claim))])

    for index, x, y, w, h in claims:
        if np.all(fabric[x: x+w, y: y+h] == 1):
            return index


if __name__ == "__main__":
    # Create the fabric sample
    fabric_sample = np.zeros((1000, 1000))
    # Read in elf claims
    elf_claims = open("puzzle_inputs/input_day_3.txt", "r").readlines()
    # Get Results
    print(calculate_overlap(elf_claims, fabric_sample), get_non_overlap_id(elf_claims, fabric_sample))
