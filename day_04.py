...
the Elves seem to have decided that one guard was enough for the overnight shift - as well as when they fall
...
the one whose shift most recently started. Because all asleep/awake times are during the midnight hour (00:00 - 00:59),
...
def generate_data_from_log(log_data: list) -> defaultdict(Counter):
    """
    Formats data to parse info from

    Arguments:
        log_data (list): List of unordered log entries for guard sleep and wake states
    """
    _guards = defaultdict(Counter)

    guard = 0
    sleep_start = datetime.now()

    for time, action in [entry.split('] ') for entry in sorted(log_data) if entry]:

        time = datetime.strptime(time, '[%Y-%m-%d %H:%M')
        # Isolate guard identification from string input
        if '#' in action:
            guard = int(action.split('#')[1].split()[0])

        # Get the sleep start time for the guard
        if 'asleep' in action:
            sleep_start = time

        # Get the wake up time span from moment guard fell asleep to waking up.
        # Update the guard's counter for sleep state
        if 'wakes' in action:
            min_asleep = int((time - sleep_start).total_seconds() // 60)
            _guards[guard].update(Counter((sleep_start.minute + i) % 60 for i in range(min_asleep)))

    return _guards


def find_worst_guard(guards: defaultdict(Counter)):
    """
    Finds the guard who should be first on the chopping block based on total number of minutes slept
    while on the job during the midnight hour

    Arguments:
        guards (defaultdict(Counter)): Generated data log of all guards on duty and time spent sleeping
    """

    worst_guard = max((sum(guard.values()), identification) for identification, guard in guards.items())[1]
    return worst_guard * guards[worst_guard].most_common()[0][0]


def find_most_consistent_guard(guards: defaultdict(Counter)):
    """
    Finds the guard who frequently sleeps during the same minute across all days the guard has a shift.

    Arguments:
        guards (defaultdict(Counter)): Generated data log of all guards on duty and time spent sleeping
    """

    (_, m), consistent_guard = max((guard.most_common()[0][::-1], identification) for identification, guard in guards.items())
    return consistent_guard * m


if __name__ == "__main__":

    # Grab data input from text file
    data_log = open("puzzle_inputs/input_day_4.txt", "r").readlines()

    # Generate guard data
    guard_data = generate_data_from_log(data_log)

    # Get results of the two guards who should be canned
    print(find_worst_guard(guard_data), find_most_consistent_guard(guard_data))
