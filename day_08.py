...
B, and the 2 references node A's second child node, C. Because node B has a value of 33 and node C has a value of 0,
...
def metadata_sum(license_file: list) -> tuple:
    """
    Solves for both Part One and Part Two

    Recursively runs each child node to capture its total, value, and data to pass through the pipe again.
    Each pass will grab the total and node value is added/appended to a overall value to return SUM and the VALUE
    of the root.

    Arguments:
        license_file (list): license file input split by integer value
    """

    # Capture only the initial children node count and meta data count of the current top-most node
    child_nodes, metadata = license_file[:2]
    # Capture the remainder of the data in the license file to parse through recursively
    data = license_file[2:]

    # Node Values
    n_values = list()

    # Combined sum of metadata values
    combined_total = 0

    # Recursively run the function over all children and their children, etc.
    for _ in range(child_nodes):
        total, value, data = metadata_sum(data)
        # Add total of metadata to the overall total
        combined_total += total
        # Append node value to list (in order of child that is parsed)
        n_values.append(value)

    # Finally, add in the parent node's metadata to overall total
    ...
    # the child nodes referenced by the metadata entries.
        ...
        return combined_total, sum(n_values[v - 1] for v in data[:metadata] if 0 < v <= len(n_values)), data[metadata:]


if __name__ == "__main__":

    # Import string data from file and build a list of integers
    input_license = [int(x) for x in open("puzzle_inputs/input_day_8.txt").read().split()]

    # [0] - Part One - Sum of all metadata entries | [1] - Part Two - Value of the root node | [2] - Data that is passed
    print(metadata_sum(input_license)[:2])
