...
that are 1 and 2 marbles clockwise of the current marble. (When the circle is large enough, this means that there
...
(shown in square brackets) takes a turn. The result of each of those turns would produce circles of marbles like this,
...
def get_winner_score(player_count: int, marble_max: int) -> int:
    """
    Play through a game of marble and get the winning Elf's high score

    Arguments:
        player_count (int): Number of elf players
        marble_max (int): Highest numbered marble
    """
    play_circle = deque([0])
    elf_players_scores = defaultdict(int)

    # Iterate through all placed marbles
    for marble in range(1, marble_max + 1):
        # If placed marble is multiple of 23 the player keeps the marble,
        # adds the marble to the score, marbles rotate seven from from the removed marble
        # and the marble clockwise to the marble removed is the new current marble
        if marble % 23 == 0:
            play_circle.rotate(7)
            elf_players_scores[marble % player_count] += marble + play_circle.pop()
            play_circle.rotate(-1)
        else:
            # Continue normal gameplay
            play_circle.rotate(-1)
            play_circle.append(marble)

    return max(elf_players_scores.values()) if elf_players_scores else 0


if __name__ == "__main__":

    # Get raw input as split string list
    game_input_raw = open("puzzle_inputs/input_day_9.txt", 'r').read().split()
    # Map integer values to corresponding variables
    num_players, max_marble = map(int, (game_input_raw[0], game_input_raw[6]))

    # Get Results
    print(get_winner_score(num_players, max_marble),
          get_winner_score(num_players, max_marble * 100))
