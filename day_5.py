...
you can do better. You scan the chemical composition of the suit's material and discover that it is formed
...
Your goal is to figure out which unit type is causing the most problems, remove all instances of
it (regardless of polarity), fully react the remaining polymer, and measure its length.
...
def polymer_extraction(polymer) -> int:
    """
    Removes units of the same type of polarity in the polymer input

    Arguments:
        polymer (str, Generator): Polymer data input
    """

    # Needs to have a non-alpha character to start process. Number works.
    reacted_polymer = ['0']

    for letter in polymer:
        last_letter = reacted_polymer[-1]

        # Removes same polarity characters if equal
        if last_letter != letter and last_letter.lower() == letter.lower():
            reacted_polymer.pop()
        # Adds current letter to new polymer string
        else:
            reacted_polymer.append(letter)

    # Return length of extracted polymer string minus the initial digit included.
    return len(reacted_polymer) - 1


def smallest_polymer_extraction(polymer: str) -> int:
    """
    Iterate through every letter unit type to find which fully reacted polymer yields the smallest length

    Arguments:
        polymer (str): Polymer data input
    """

    extraction = (polymer_extraction(letter for letter in polymer if letter.lower() != ch) for ch in ascii_lowercase)
    return min(extraction)


if __name__ == "__main__":

    _polymer = open("puzzle_inputs/input_day_5.txt").read().strip()  # Have to read it as a single entity
    print(polymer_extraction(_polymer), smallest_polymer_extraction(_polymer))
